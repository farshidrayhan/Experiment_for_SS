...
total_matrix = pandas.DataFrame(pandas.read_csv('/home/farshid/Desktop/nr_dataset_1477.txt'))
...
# sampler = InstanceHardnessThreshold(estimator=RandomForestClassifier(n_estimators=150,max_depth=18,min_samples_split=2))
...
for c in numpy.arange(2400, 3000, 10):
    for g in numpy.arange(.0029, .0035, .0001):
        # for score in scores:
        ...
        for train_set_indexes, test_set_indexes in kf.split(feature_list_of_all_instances, class_list_of_all_instances):
            ...
            # print("negative in test list ",
            #       cou2)  # clf = GridSearchCV(SVC(), tuned_parameters, n_jobs=4, pre_dispatch='4*n_jobs', cv=5,
            #                    scoring='%s' % score)
        ...
        # break  # fold_counter += 1  # if top_roc >= .79:  # random_state_list.append(random_state)

        if avg_roc/5 > top_roc:
            top_roc = avg_roc/5
