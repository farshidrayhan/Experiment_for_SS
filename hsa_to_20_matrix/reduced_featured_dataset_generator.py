...
print("Opening  list csv ...  ")

selected_list = []
with open('list.csv', newline='') as csvfile:
    spamreader = csv.reader(csvfile, delimiter=' ', quotechar='|')
    for row in spamreader:
        # print(', '.join(row))  # file_read = open('dataset_test.txt', 'r')
        selected_list.append(row[0])
...
with open('dataset_test.txt', 'r') as file_read:
    for x in file_read:
        ...
        #     if selected_list.__contains__(i):
        #         new_l.append(l[i])
        # feature_list_of_all_instances.append(new_l)
        ...
        #     break
print("length of total matrix " , len(total_matrix) , " count for total positive class  ", ii )
# feature_list_of_all_instances = standardalize.std(feature_list_of_all_instances, 238, 280)
# print("length of total matrix " , len(total_matrix) , " ", i )/
# print("reducing features ...")
# for i in range(0,len(total_matrix)):
#     temp_list = []
#     if i % 500 == 0:
#         print(i," ",end='')
#     for j in range(0, len(total_matrix[i])):
#
#         if j ==  1294:
#             class_list_of_all_instances.append(total_matrix[i][j])
#         if  selected_list.__contains__(j):
#             temp_list.append(total_matrix[i][j])
#     feature_list_of_all_instances.append(temp_list)
# total_matrix = []
...
target = open("feature_reduced_dataset.txt", 'w')
for index in range(0,len(feature_list_of_all_instances)):
    str1 = ','.join(str(i) for i in feature_list_of_all_instances[index]   )
    str2 = str(class_list_of_all_instances[index]   )
    write = str1 + "," + str2 + "\n"
    target.write(write)
...
#             temp_train_feature_list.append(feature_list_of_all_instances[index][0:519])
...
#             temp_train_feature_list.append(feature_list_of_all_instances[cursor][0:519])
#             temp_train_class_list.append(class_list_of_all_instances[cursor])
#             # temp_total_dataset.append(feature_list_of_all_instances[index][0:1282])
#
#     for index in test_set_indexes:
#         temp_test_feature_list.append(feature_list_of_all_instances[index][0:519])
...
#         print()
#
#         clf = GridSearchCV(SVC(C=1), tuned_parameters, cv=5, scoring='%s' % score,n_jobs=4)
#         #  ^   cv=5
#         #                           ,n_jobs=-1
