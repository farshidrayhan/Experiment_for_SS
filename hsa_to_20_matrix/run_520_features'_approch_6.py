...
# sampler = InstanceHardnessThreshold(estimator=RandomForestClassifier(n_estimators=50,max_depth=15,min_samples_split=5))
...
for train_set_indexes, test_set_indexes in kf.split(feature_list_of_all_instances, class_list_of_all_instances):
    ...
    #
    est = DecisionTreeClassifier(max_depth=150)  # roc avg result  = 0.895951148384 cross fold = 5 l_rate = 1 n_estimator = 150
    ...
    for score in scores:
        ...
        clf = GridSearchCV(AdaBoostClassifier(), tuned_parameters, n_jobs=8, pre_dispatch='4*n_jobs', cv=5,
                           scoring='%s' % score)
    ...
    # print(sklearn.metrics.confusion_matrix(temp_test_class_list, predicted_with_threshold))
        ...
        print(average_precision_score(temp_test_class_list, predicted[:, 1]))
