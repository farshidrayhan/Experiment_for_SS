...
for iteration, data in enumerate(kf, start=1):
    ...
    for index in train_set_indexes:
        if class_list_of_all_instances[index] == 1:
            temp_train_feature_list.append(feature_list_of_all_instances[index][0:1282])
    ...
    for cursor in randomly_generated_indexes:
        if class_list_of_all_instances[cursor] == 0:
            temp_train_feature_list.append(feature_list_of_all_instances[cursor][0:1282])
            temp_train_class_list.append(class_list_of_all_instances[cursor])
            temp_total_dataset.append(feature_list_of_all_instances[index][0:1282])

    for index in test_set_indexes:
        temp_test_feature_list.append(feature_list_of_all_instances[index][0:1282])
    ...
    gamma_list = []
    tol_range = []
    for x in range_custom(.01, 1, .5):
        C_List.append(x)

    for x in range_custom(1, 10, 1):
        gamma_list.append(x)
    ...
    scores = [ 'average_precision','precision_macro' ,'recall_macro',]
    ...
    for score in scores:
        ...
        clf = GridSearchCV(svm.OneClassSVM(nu=1), tuned_parameters, cv=5, scoring='%s' % score)
        #                           ,n_jobs=-1
