...
for i in hsa_list:
    ...
    break
    # query = "insert into hsa_structure  values (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
    # query = "insert into hsa_structure_2  values (%s, %s)"

    # data = (hsa,datas)
    # (`hsa`, `C`, `E`, `H`, `ASA`, `psi_sin`, `psi_cos`, `phi_sin`, `phi_cos`, `theta_sin`, `theta_cos`, `tau_sin`, `tau_cos`)
    # b = cursor.execute(query,data)
    # cnx.commit()
    # cnx.close
    # print("c count " ,C_count,"\n h count",H_count,"\n e count",E_count,"\n ASA sum",ASA_sum,"\n length",length_counter,"\n ASA/L",ASA_sum/length_counter)
    # print("phi_sin_sum ", phi_sin_sum, "\n phi_cos_sum", phi_cos_sum )
